...
```bash
docker run --rm -it -p 8888:8888 --name steeltoe-config steeltoe.azurecr.io/config-server
```
...
docker run --rm -it -p 8888:8888 -v $PWD/steeltoe/config-repo:/config --name steeltoe-config steeltoe.azurecr.io/configserver --spring.profiles.active=native
```

To start a config server that registers itself with Eureka at startup (discovery-first):

```bash
docker run --rm -it -p 8888:8888 --name steeltoe-config -e eureka.client.enabled=true steeltoe.azurecr.io/config-server
```
...
```bash
docker run --rm -it -p 8761:8761 --name steeltoe-eureka steeltoe.azurecr.io/eureka-server
```
...
```script
docker run --rm -it -p 5672:5672 -p 15672:15672 --name rabbitmq rabbitmq:3-management
```
...
```script
docker run --rm -it -p 8500:8500 --name=steeltoe-consul hashicorp/consul
```
...
```script
docker run --rm -it -p 8080:8080 --name steeltoe-springbootadmin steeltoe.azurecr.io/spring-boot-admin
```
...
```script
docker run --rm -it -p 1433:1433 --name steeltoe-sqlserver -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=St33ltoeR0cks!' mcr.microsoft.com/mssql/server
```
...
```script
docker run --rm -it -p 5432:5432 --name steeltoe-postgres -e POSTGRES_DB=steeltoe -e POSTGRES_USER=steeltoe -e POSTGRES_PASSWORD=steeltoe postgres:alpine
```
...
```script
docker run --rm -it -p 27017:27017 --name mongoserver mongo
```
